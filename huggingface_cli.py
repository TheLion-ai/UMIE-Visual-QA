...
@click.option(
    "--dataset-name",
    prompt="Select dataset to download",
...
def download(dataset_name: str, output_dir: str) -> None:
    """
    Download a dataset from the Hugging Face Hub.

    Args:
        dataset_name (str): The name of the dataset to download.
        output_dir (str): The directory to save the downloaded dataset.
    """
    dataset = load_dataset(hf_repo_name, dataset_name)
    
    os.makedirs(output_dir, exist_ok=True)
    
    dataset.save_to_disk(os.path.join(output_dir, dataset_name))
    print(f"Dataset '{dataset_name}' has been downloaded to {output_dir}")
...
if __name__ == "__main__":
    ...
    @click.group()
    def cli() -> None:
        """Hugging Face CLI for uploading and downloading datasets to/from the Hub."""
        pass
